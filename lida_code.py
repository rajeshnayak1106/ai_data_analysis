...
if openai_key and selected_dataset and selected_method:
    ...
    textgen_config = TextGenerationConfig(
        n=1, temperature=temperature, model=selected_model, use_cache=use_cache
    )

    st.write("## Summary")
    # **** lida.summarize *****
    summary = lida.summarize(
        selected_dataset, summary_method=selected_method, textgen_config=textgen_config
    )
    ...
    if summary:
        ...
        num_goals = st.sidebar.slider(
            "Number of goals to generate", min_value=1, max_value=10, value=4
        )
        ...
        if own_goal:
            ...
            if user_goal:
                ...
                new_goal = Goal(
                    question=user_goal, visualization=user_goal, rationale=""
                )
